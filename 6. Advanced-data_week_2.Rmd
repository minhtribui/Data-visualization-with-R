...
ggplot(data = map_data_combined, mapping = aes(x= long, y= lat,group=group, fill = Score))+
  geom_polygon( )  +
  scale_fill_distiller(palette=5)
```
...
prob2 = filter(map_data_combined, region %in% c("Costa Rica", "Panama", "Nicaragua" ))

my_cities <-maps::world.cities
#overforty1 <- filter(my_cities, country.etc == "Nicaragua" & pop > 40000 )
#overforty2 <- filter(my_cities, country.etc =="Panama" & pop > 40000 )
#overforty3 <- filter(my_cities, country.etc =="Costa Rica"  & pop > 40000 )
#overforty <-rbind(overforty1, overforty2, overforty3)

overforty <- filter(my_cities, country.etc %in% c("Costa Rica", "Panama", "Nicaragua") & pop >= 40000)
overforty = add_column(overforty, Measurement )

ggplot(data = prob2, mapping = aes(x= long, y= lat,group= group))+
  geom_polygon(colour = "black", fill = "white" ) +
  geom_point(data = overforty, aes(x= long, y= lat, group = NULL, colour = Measurement), size = 5) + scale_color_distiller(palette=7)   
```
...
####PUT YOUR CODE HERE
ggplot(data= s_america)+geom_sf(aes(fill = pop_est))+
