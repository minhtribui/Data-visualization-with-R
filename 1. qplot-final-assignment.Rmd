...
```{r,problem4}
boxplot(dat$faminc_new, data = dat, xlab = "Family Income", main = "Collin's Boxplot of Family Income")    
```
...
a= filter(dat, gender == 1 & region <3 & marstat == 1 & newsint == 1)
str(a)
```
...
table(filter(dat, gender == 1 & region <3 & marstat == 1 & newsint == 1)$investor)
``` 
...
```{r problem10}
q10 <- filter(dat , faminc_new == 5 | faminc_new == 6 | faminc_new == 7 | faminc_new == 8 | faminc_new == 9 | faminc_new ==10 , investor == 2)

q10 %>% summarise(Mean_imp_religion = mean(q10$pew_religimp), Median_imp_religion = median(q10$pew_religimp) , Standard_Dev_Religion_Imp = sd(pew_religimp))
```
